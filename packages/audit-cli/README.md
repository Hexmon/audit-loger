...
```bash
pnpm --filter @yourorg/audit-cli exec audit-cli verify-postgres \
...
```bash
pnpm --filter @yourorg/audit-cli exec audit-cli export-postgres \
  --connection "postgres://user:pass@localhost:5432/audit" \
  --from "2024-01-01" \
  --to "2024-01-31" \
  --tenantId "tenant-123" \
...
```bash
pnpm --filter @yourorg/audit-cli exec audit-cli retention-postgres \
  --connection "postgres://user:pass@localhost:5432/audit" \
  --before "2024-01-01" \
  --dry-run
```
...
## Notes
- Verification uses the same hash chain algorithm as `@yourorg/audit-core`.
